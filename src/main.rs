use std::convert::From;
use std::sync::mpsc::sync_channel;
use std::io;

const MAX_MIDI: usize = 3;

#[derive(Copy, Clone)]
struct MidiCopy {
    len: usize,
    data: [u8; MAX_MIDI],
    time: jack::Frames,
}

impl From<jack::RawMidi<'_>> for MidiCopy {
    fn from(midi: jack::RawMidi<'_>) -> Self {
        let len = std::cmp::min(MAX_MIDI, midi.bytes.len());
        let mut data = [0; MAX_MIDI];
        data[..len].copy_from_slice(&midi.bytes[..len]);
        MidiCopy{
            len,
            data,
            time: midi.time,
        }
    }
}

impl std::fmt::Debug for MidiCopy {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        write!(
            f,
            "Midi {{ time: {}, len {}, data: {:?} }}",
            self.time,
            self.len,
            &self.data[..self.len]
            )
    }
}

fn main() {
    let (client, _status) = jack::Client::new("rust_jack_midi", jack::ClientOptions::NO_START_SERVER).unwrap();

    println!("Select a port");
    let ports = client.ports(None, None, jack::PortFlags::IS_OUTPUT);
    let mut i = 0;
    for port in &ports {
        println!("{}: {}", i, port);
        i += 1;
    }

    let mut raw_port_choice = String::new();
    io::stdin().read_line(&mut raw_port_choice).ok();
    println!("{}", raw_port_choice);
    let port_choice = raw_port_choice.trim().parse::<usize>().unwrap();

    let shower = client.register_port("rust_midi_shower", jack::MidiIn::default())
        .unwrap();

    let (sender, reciver) = sync_channel(64);

    let cback = move |c: &jack::Client, ps: &jack::ProcessScope| -> jack::Control {
        let source_port = c.port_by_name(&ports[port_choice]).unwrap();
        let shower_name = shower.name().unwrap();
        if !source_port.is_connected_to(&shower_name).unwrap() {
            c.connect_ports(&source_port, &shower).unwrap();
        }
        let show_p = shower.iter(ps);
        for e in show_p {
            let c: MidiCopy = e.into();
            let _ = sender.try_send(c);
        }

        jack::Control::Continue
    };

    let activate_client = client.activate_async((), jack::ClosureProcessHandler::new(cback)).unwrap();


    std::thread::spawn(move || {
        while let Ok(m) = reciver.recv() {
            println!("{:?}", m)
        }
    });

    println!("Press any key to quit");
    let mut user_input = String::new();
    io::stdin().read_line(&mut user_input).ok();

    activate_client.deactivate().unwrap();
}
